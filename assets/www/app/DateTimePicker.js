Ext.ux.DateTimePicker = Ext.extend(Ext.Picker, {
     ...
     */
    slotOrder: ['relativeFormat','hour','minute','daynight'],
    ...
    initComponent:function() {
        ...
        for (i=0; i<=365; i++)
        {
            var offset=i-182;
            var myDate = new Date();
            //var removeTime=new Date(0, 0, 0, myDate.getHours(), myDate.getMinutes(),myDate.getSeconds(),myDate.getMilliseconds())
            var date=new Date(myDate.setDate(myDate.getDate() + offset));
            if(offset==0)
            {
                relativeFormat.push({
                    text: 'Today',
        ...
        for (i = 0, ln = Date.monthNames.length; i < ln; i++) {
            months.push({
                text: Date.monthNames[i],//.substring(0,3)
                value: i + 1
            });
        }
        ...
        Ext.each(this.slotOrder, function(item){
            this.slots.push(this.createSlot(item, relativeFormat,days, months, years,hours,minutes,daynight));
        }, this);
    ...
    createSlot: function(name,relativeFormat, days, months, years,hours,minutes,daynight){
        switch(name){
            case 'relativeFormat':
                return {
                    name: 'relativeFormat',
                    align: 'center',
                    data: relativeFormat,
                    title: this.useTitles ? this.relativeFormatText : false,
                    flex: 3
                };
            case 'year':
                return {
    ...
    getValue: function() {
        ...
        var date=new Date(value.relativeFormat);
    ...
    setValue : function(values, animated) {
        ...
        //alert(this.child('[name=relativeFormat]').getValue());
